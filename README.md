...
- Tab insertion/removal uses Chromium's 18px overlap endpoint and a 100ms linear interpolation; closing mirrors opening. Once tabs are compressed, closing a non-trailing tab preserves their current width and close-button position across consecutive closes until the pointer leaves the full tab area. New tabs expand toward available space and keep their right edge fixed once the strip reaches its reserved 40px trailing margin.
